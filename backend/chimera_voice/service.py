...
class VoiceService:
    ...
    def vobiz_opening(self, intervention_id: str) -> str:
        """Prepare the first live Vobiz turn and return the Hinglish prompt."""
        call = self.get_call_for_intervention(intervention_id)
        if call.status in TERMINAL_VOICE_STATUSES:
            return "यह recovery call पहले ही complete हो चुकी है। धन्यवाद।"
        context = build_voice_context(call.intervention, allow_secondary=True)
        if not call.turns:
            self._record_turn(call, VoiceAgent(context).opening_turn(self._now()), context)
        self._bring_to_conversation(call, source="vobiz")
        self.session.commit()
        return call.turns[0].text

    def vobiz_stream_opened(self, intervention_id: str, stream_id: str | None = None) -> VoiceCall:
        """Persist the point at which Vobiz connects the audio stream."""
        call = self.get_call_for_intervention(intervention_id)
        if call.status in TERMINAL_VOICE_STATUSES:
            return call
        self._bring_to_conversation(call, source="vobiz")
        payload = {"stream_connected": True}
        if stream_id:
            payload["stream_id"] = stream_id[:128]
        self._event(call, "STREAM_CONNECTED", "vobiz", payload)
        self.session.commit()
        return self.get_call(call.id)

    def vobiz_stream_failed(self, intervention_id: str | None, *, stage: str, code: str, message: str | None = None) -> None:
        """Persist a visible bridge/provider failure for Vobiz stream."""
        if not intervention_id:
            return
        try:
            call = self.get_call_for_intervention(intervention_id)
        ...
        if call.status not in TERMINAL_VOICE_STATUSES:
            ...
            call.completed_at = self._now()
            self._transition(call, VoiceCallStatus.FAILED)
        self._event(call, "VOICE_STREAM_FAILED", "sarvam" if stage == "sarvam" else "vobiz", payload)
        self.session.commit()

    def handle_vobiz_transcript(self, intervention_id: str, transcript: str) -> tuple[str, bool]:
        """Apply one transcript to the controlled recovery conversation via Vobiz."""
        return self._handle_customer_text(intervention_id, transcript, source="vobiz", complete=False)

    def get_voice_context(self, intervention_id: str) -> VoiceContext:
        call = self.get_call_for_intervention(intervention_id)
        return build_voice_context(call.intervention, allow_secondary=True)

    def get_recent_history(self, intervention_id: str) -> list[dict[str, str]]:
        call = self.get_call_for_intervention(intervention_id)
        return [{"speaker": turn.speaker, "text": turn.text} for turn in call.turns]

    def ensure_payment_link_sent(self, intervention_id: str) -> str | None:
        """Create and deliver a payment link for the customer immediately via Razorpay / messaging."""
        call = self.get_call_for_intervention(intervention_id)
        if self.payment_service is None:
            return None
        ...
        try:
            ...
            call.payment_link = payment.short_url
            self._event(call, "PAYMENT_LINK_ATTACHED", "payment_service", {"payment_id": payment.id, "provider": payment.provider})
        except Exception as exc:
            logger.warning("Failed creating payment link: %s", exc)

        if payment and self.messaging_service is not None:
            try:
                message = self.messaging_service.send_for_voice_link(call.intervention_id, payment.short_url)
                self._event(call, "PAYMENT_LINK_NOTIFIED", "messaging_service", {"message_id": message.id, "provider": message.provider, "provider_message_id": message.provider_message_id})
            except Exception as exc:
                self._event(call, "PAYMENT_LINK_NOTIFICATION_FAILED", "messaging_service", {"provider": getattr(self.messaging_service.provider, "name", "unknown"), "error": str(exc)[:128]})

        self.session.commit()
        return call.payment_link

    def record_vobiz_turn(self, intervention_id: str, transcript: str, response_text: str) -> bool:
        """Record customer and agent turns when generated by an external LLM (e.g. Groq)."""
        call = self.get_call_for_intervention(intervention_id)
        if call.status in TERMINAL_VOICE_STATUSES:
            return True
        self._bring_to_conversation(call, source="vobiz")
        context = build_voice_context(call.intervention, allow_secondary=True)
        agent = VoiceAgent(context)
        cust_turn = agent.customer_turn(transcript.strip(), self._now())
        self._record_turn(call, cust_turn, context)
        self._event(
            call,
            "SPEECH_TRANSCRIBED",
            "sarvam",
            {
                ...
                "language_code": getattr(self.speech_provider, "language_code", "hi-IN"),
                "model": getattr(self.speech_provider, "stt_model", "saaras:v3"),
            },
        )
        intent = cust_turn.intent or VoiceIntent.UNKNOWN
