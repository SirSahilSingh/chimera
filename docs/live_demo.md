...
- For a real Hinglish conversation, enable Exotel AgentStream with `EXOTEL_AGENTSTREAM_ENABLED=true`, `EXOTEL_STREAM_URL=wss://<public-host>/api/v1/voice/exotel/stream`, `SARVAM_ENABLED=true`, and `SARVAM_API_KEY`. AgentStream must be enabled for the Exotel account. The bridge receives Exotel's 8 kHz PCM bidirectional stream, sends completed utterances to Sarvam Saaras `codemix`, and returns Sarvam Bulbul telephony audio to the caller.
