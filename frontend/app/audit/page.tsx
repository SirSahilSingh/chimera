...
import { IntelligenceTitle, IntelEmpty } from "../../components/intelligence-workspace";
...
export default function AuditPage() {
    ...
    try {
      ...
      const rows = await Promise.all(response.items.map(async (item) => {
        try {
          ...
          return journey.audit_trail.map((event) => ({ ...event, caseRef: item.external_event_id || item.id, caseId: item.id, caseStatus: item.status }));
        } catch {
          return [];
        }
      }));
  ...
  return <div className="system-page">
    <IntelligenceTitle title="Audit Trail" action={<button className="square-control" type="button" onClick={load} disabled={loading} aria-label="Refresh audit trail"><RefreshIcon size={16} /></button>} />
...
function AuditFilter({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
  return <DropdownField className="audit-filter" label={label} value={value} onChange={onChange} options={[{ value: "", label: "All" }, ...options.map((option) => ({ value: option, label: option.replaceAll("_", " ") }))]} />;
}
