...
const scenarios = [
  ...
  { value: "voice_recovery", label: "Voice-Assisted Recovery", summary: "Live Hinglish outbound phone call", setup: "An abandoned payment recovered through a live Vobiz + Sarvam + Groq phone call.", evidence: "Live transcript, telephony stream, and persisted recovery evidence", icon: FlaskIcon },
] as const;
...
function RunMonitor({ run, journey }: { run: DemoRunResponse; journey: RecoveryJourney }) {
  ...
  const stages = [
    ...
    { label: browserVoice ? "Vobiz Outbound Voice" : "Provider action", complete: browserVoice || hasProviderAction, detail: browserVoice ? "Vobiz + Sarvam outbound session ready" : hasProviderAction ? `${run.provider_mode_label} · persisted receipt available` : "No provider action recorded" },
