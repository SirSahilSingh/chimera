...
import { FailureBreakdown, RecoveryActivityFeed, RootCauseInsight } from "../components/operational";
...
export default function CommandCenter() {
  ...
  const load = async () => { setLoading(true); setError(null); try { const response = await api.listCases({ page: 1, pageSize: 100 }); setCases(response.items); setTotal(response.total); } catch (err) { setError(err instanceof ApiError ? err.detail : "Could not load recovery operations."); } finally { setLoading(false); } };
  useEffect(() => { load(); }, []);
