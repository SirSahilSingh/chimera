...
import { formatAction, formatDate, formatPercent } from "../../../lib/formatters";
import { ArrowRightIcon, CheckIcon, RefreshIcon } from "../../../components/icons";
...
export default function DecisionEnginePage() {
  ...
  return <div className="system-page">
    <IntelligenceTitle title="Decision Engine" action={<button className="square-control" type="button" onClick={load} disabled={loading} aria-label="Refresh decision engine"><RefreshIcon size={16} /></button>} />
    <section className="engine-metrics"><EngineMetric label="Stored decisions" value={String(decisions.length)} note="Persisted decision records" /><EngineMetric label="Latest action" value={latest ? formatAction(latest.selected_action) : "—"} note={latest ? `Recorded ${formatDate(latest.created_at)}` : "No stored decision"} /><EngineMetric label="Predicted recovery" value={latest ? formatPercent(latest.predicted_probability) : "—"} note="Latest stored prediction" /></section>
    {!latest ? <IntelEmpty label="No stored decisions are available yet. Decision provenance will appear after a recovery case is evaluated." /> : <>
      <div className="system-two-column"><IntelligencePanel title="How the engine selects" note="Stored behavior"><div className="engine-rule-list"><EngineRule text="Scores observable recovery context." /><EngineRule text="Applies action cost and fatigue treatment." /><EngineRule text="Removes candidates blocked by policy constraints." /><EngineRule text="Persists the selected permissible action." /></div></IntelligencePanel><IntelligencePanel title="Latest selection" note="Read-only case link"><div className="selection-readout"><span>Selected action</span><strong>{formatAction(latest.selected_action)}</strong><div className="selection-bar"><i style={{ width: `${Math.max(4, Math.min(100, latest.predicted_probability * 100))}%` }} /></div><small>{formatPercent(latest.predicted_probability)} predicted recovery</small></div><Link className="engine-link" href={`/cases/${latest.recovery_case_id}`}>Open Decision Room <ArrowRightIcon size={14} /></Link></IntelligencePanel></div>
