  ...
  return <div className="voice-recovery-page">
    ...
    <section className="voice-context-card">
      ...
      <form className="voice-context-form" onSubmit={startRecovery}>
        ...
        <DropdownField label="Failure Reason" required labelAbove value={failureReason} onChange={setFailureReason} options={failureReasons.map(([value, label]) => ({ value, label }))} helperText="Observed checkout failure reason." />
        <DropdownField label="Payment Method" required labelAbove value={paymentMethod} onChange={(value) => setPaymentMethod(value as "card" | "upi" | "netbanking")} options={paymentMethods.map(([value, label]) => ({ value, label }))} helperText="Customer payment instrument used." />
