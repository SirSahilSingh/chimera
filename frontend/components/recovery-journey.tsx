...
export function ProviderJourney({ journey, customerPhone }: { journey: RecoveryJourney; customerPhone?: string | null }) {
  ...
  return <section className="provider-journey" aria-label="Provider operations">
    ...
    <div className="provider-grid">
      ...
      {messages.map((message) => <OperationRow key={message.id} title={message.delivery_state === "DELIVERED" ? "Message delivered" : "Message delivery"} provider={message.provider} mode={message.provider_mode} status={message.delivery_state} detail={`${message.delivery_state === "DELIVERED" ? "Delivered to" : "Delivery pending for"} ${customerPhone ? maskPhone(customerPhone) : "stored customer number"}${message.failure_reason ? ` · ${message.failure_reason}` : ""}`} />)}
